...
Ext.ux.tree.TreeGridEditorNodeUI = Ext.extend(Ext.ux.tree.TreeGridNodeUI, {
    renderElements: function(n, a, targetNode, bulkRender) {
        ...
        buf = ['<tbody class="x-tree-node">', '<tr treegrideditornodeid="', n.id, '" ext:tree-node-id="', n.id, '" class="x-tree-node-el ', a.cls, '">', '<td class="x-treegrid-col ', (c.cls ? c.cls : ''), '">', '<span class="x-tree-node-indent">', this.indentMarkup, "</span>", '<img src="', this.emptyIcon, '" class="x-tree-ec-icon x-tree-elbow">', '<img src="', a.icon || this.emptyIcon, '" class="x-tree-node-icon', (a.icon ? " x-tree-node-inline-icon" : ""), (a.iconCls ? " " + a.iconCls : ""), '" unselectable="on">', (t.checkbox ? (cb ? ('<input id="x-treegrideditor-node-cb" name="x-treegrideditor-node-cb" class="x-tree-node-cb x-treegrideditor-node-cb" type="' + (t.checkMode === 'single' ? 'radio' : 'checkbox') + '"' + (a.checked ? ' checked="checked"' : '') + ' value="' + n.id + '"' + '/>') : '') : ''), '<span id="', n.id, '-inputfield-', c.dataIndex, '" treegrideditorinputfield="', c.dataIndex, '" unselectable="on"', (c.displayTip && cTip ? ' qtip="' + cTip + '"' : ''), '>', (c.tpl ? c.tpl.apply(nAttr) : fVal), '</span>', '</td>'];
        ...
        for (i = 1, len = cols.length; i < len; i++) {
            ...
            buf.push('<td class="x-treegrid-col ', (c.cls ? c.cls : ''), '">', '<div id="', n.id, '-inputfield-', c.dataIndex, '" treegrideditorinputfield="', c.dataIndex, '" unselectable="on" class="x-treegrid-text"', (c.align ? ' style="text-align: ' + c.align + ';"' : ''), (c.displayTip && cTip ? ' qtip="' + cTip + '"' : ''), '>', (c.tpl ? c.tpl.apply(nAttr) : fVal), '</div>', '</td>');
        }
