	...
	function e(l, n, m, p, k, i) {
		...
		if (d.useDom) {
			...
			if (i) {
				...
				(k == "firstChild" ? l : l.parentNode).insertBefore(j, l[k]
								|| l)
			}
		} else {
...
Ext.EventManager = function() {
	...
	function j(y, A, D, z, x, F) {
		...
		E[A] = E[A] || [];
		E[A].push([D, x, F, B, z]);
		if (y.addEventListener && A == "mousewheel") {
	...
	var r = {
		...
		removeListener : function(y, C, G, H) {
			y = Ext.getDom(y);
			var w = k(y), E = y && (Ext.elCache[w].events)[C] || [], x, B, z, A, D, F;
			for (B = 0, D = E.length; B < D; B++) {
				if (Ext.isArray(F = E[B]) && F[0] == G && (!H || F[2] == H)) {
					if (F[4]) {
						F[4].cancel()
					}
					...
					x = F[1];
					l.un(y, C, l.extAdapter ? F[3] : x);
					if (x && y.addEventListener && C == "mousewheel") {
						y.removeEventListener("DOMMouseScroll", x, false)
					}
					if (x && y == b && C == "mousedown") {
						Ext.EventManager.stoppedMouseDownEvent
								.removeListener(x)
					}
					E.splice(B, 1);
					if (E.length === 0) {
						delete Ext.elCache[w].events[C]
					}
					for (A in Ext.elCache[w].events) {
						return false
					}
		...
		removeAll : function(y) {
			y = Ext.getDom(y);
			var x = k(y), D = Ext.elCache[x] || {}, G = D.events || {}, C, B, E, z, F, A, w;
			for (z in G) {
				if (G.hasOwnProperty(z)) {
					...
					for (B = 0, E = C.length; B < E; B++) {
						...
						if (F[0].tasks && (A = F[0].tasks.length)) {
							while (A--) {
								F[0].tasks[A].cancel()
							}
							delete F.tasks
						...
						w = F[1];
						l.un(y, z, l.extAdapter ? F[3] : w);
						if (y.addEventListener && w && z == "mousewheel") {
							y.removeEventListener("DOMMouseScroll", w, false)
						}
						if (w && y == b && z == "mousedown") {
							Ext.EventManager.stoppedMouseDownEvent
									.removeListener(w)
						}
					}
...
Ext.apply(Ext.EventManager, function() {
	var c, j, e, b, a = Ext.lib.Dom, i = /^(?:scope|delay|buffer|single|stopEvent|preventDefault|stopPropagation|normalized|args|delegate)$/, h = 0, g = 0, d = Ext.isWebKit
...
Ext.layout.AnchorLayout = Ext.extend(Ext.layout.ContainerLayout, {
			...
			onLayout : function(l, o) {
				...
				var n = this.getRenderedItems(l), m = n.length, j, p, r, g, b, e, t;
				for (j = 0; j < m; j++) {
					p = n[j];
					e = p.getPositionEl();
					if (p.anchor) {
...
Ext.layout.ColumnLayout = Ext.extend(Ext.layout.ContainerLayout, {
			...
			onLayout : function(d, j) {
				...
				var m = o.width - this.scrollOffset, b = o.height, n = m;
				this.innerCt.setWidth(m);
				for (a = 0; a < g; a++) {
					l = e[a];
					if (!l.columnWidth) {
						n -= (l.getWidth() + l.getPositionEl().getMargins("lr"))
					}
				}
				n = n < 0 ? 0 : n;
				for (a = 0; a < g; a++) {
					...
					if (l.columnWidth) {
						l.setSize(Math.floor(l.columnWidth * n)
								- l.getPositionEl().getMargins("lr"))
					}
				}
				if (Ext.isIE) {
					if (a = j.getStyle("overflow") && a != "hidden"
							&& !this.adjustmentPass) {
...
Ext.layout.BoxLayout = Ext.extend(Ext.layout.ContainerLayout, {
			...
			renderAll : function(a, b) {
				...
				Ext.layout.BoxLayout.superclass.renderAll.call(this, a,
						this.innerCt)
			},
			onLayout : function(a, b) {
				this.renderAll(a, b)
...
Ext.layout.VBoxLayout = Ext.extend(Ext.layout.BoxLayout, {
	align : "left",
	type : "vbox",
	onLayout : function(j, K) {
		Ext.layout.VBoxLayout.superclass.onLayout.call(this, j, K);
		var k = this.getRenderedItems(j), m = k.length, I, D, o, s, z, p, v, r, e, A = this
				.getLayoutTargetSize(), q = A.width, F = A.height
				- this.scrollOffset, B = this.padding.left, u = this.padding.top, n = this.pack == "start", x = 0, H = 0, C = 0, g = 0, y = 0, G = [], E = [];
		for (D = 0; D < m; D++) {
			I = k[D];
			o = I.margins;
			z = o.top + o.bottom;
			H = Math.max(H, I.getWidth() + o.left + o.right)
		}
		var a = H + this.padding.left + this.padding.right;
		switch (this.align) {
			case "stretch" :
				this.innerCt.setSize(q, F);
				break;
			case "stretchmax" :
			case "left" :
				this.innerCt.setSize(a, F);
				break;
			case "center" :
				this.innerCt.setSize(q = Math.max(q, a), F);
				break
		}
		var d = Math.max(0, q - this.padding.left - this.padding.right);
		for (D = 0; D < m; D++) {
			I = k[D];
			o = I.margins;
			if (this.align == "stretch") {
				I
						.setWidth(((q - (this.padding.left + this.padding.right)) - (o.left + o.right))
								.constrain(I.minWidth || 0, I.maxWidth
												|| 1000000))
			} else {
				if (this.align == "stretchmax") {
					I.setWidth((H - (o.left + o.right)).constrain(I.minWidth
									|| 0, I.maxWidth || 1000000))
				} else {
					if (n && I.flex) {
						I.setWidth()
					}
		...
		for (D = 0; D < m; D++) {
			...
			C += I.flex || 0;
			if (!I.flex) {
				if (!I.height && !I.hasLayout && I.doLayout) {
					I.doLayout()
				}
				s = I.getHeight()
			} else {
		...
		e = Math.max(0, (F - x - this.padding.top - this.padding.bottom));
		var J = e;
		for (D = 0; D < m; D++) {
			I = k[D];
			if (n && I.flex) {
				s = Math.floor(e * (I.flex / C));
				J -= s;
				G.push(s)
			}
		}
		...
		for (D = 0; D < m; D++) {
			...
			if (this.align == "center") {
				if ((v = d - (I.getWidth() + o.left + o.right)) > 0) {
					p += (v / 2);
					r -= v
				}
			}
...
Ext.layout.HBoxLayout = Ext.extend(Ext.layout.BoxLayout, {
	...
	onLayout : function(e, J) {
		Ext.layout.HBoxLayout.superclass.onLayout.call(this, e, J);
		var g = this.getRenderedItems(e), j = g.length, H, D, n, d, q, s, A, v = this
				.getLayoutTargetSize(), p = v.width - this.scrollOffset, F = v.height, y = this.padding.left, r = this.padding.top, m = this.pack == "start", C = [
				"stretch", "stretchmax"].indexOf(this.align) == -1, G = 0, x = 0, z = 0, k = 0;
		for (D = 0; D < j; D++) {
			H = g[D];
			z += H.flex || 0;
			if (!H.flex) {
				if (!H.width && !H.hasLayout && H.doLayout) {
					H.doLayout()
				}
				d = H.getWidth()
			} else {
			...
			n = H.margins;
			G += d + n.left + n.right;
			x = Math.max(x, H.getHeight() + n.top + n.bottom)
		}
		A = Math.max(0, (p - G - this.padding.left - this.padding.right));
		var B = x + this.padding.top + this.padding.bottom;
		switch (this.align) {
			case "stretch" :
				this.innerCt.setSize(p, F);
				break;
			case "stretchmax" :
			case "top" :
				...
				break;
			case "middle" :
				this.innerCt.setSize(p, F = Math.max(F, B));
				break
		}
		var I = A, o = [], E = [], u = 0, b = Math.max(0, F - this.padding.top
						- this.padding.bottom);
		for (D = 0; D < j; D++) {
			H = g[D];
			if (m && H.flex) {
				d = Math.floor(A * (H.flex / z));
				I -= d;
				o.push(d)
			}
		}
		...
		for (D = 0; D < j; D++) {
			...
			n = H.margins;
			y += n.left;
			H.setPosition(y, r + n.top);
			if (m && H.flex) {
				d = Math.max(0, o[u++] + (I-- > 0 ? 1 : 0));
				if (C) {
					E.push(H.getHeight())
				}
				H.setSize(d, b)
		...
		for (D = 0; D < j; D++) {
			...
			if (this.align == "stretch") {
				H
						.setHeight(((F - (this.padding.top + this.padding.bottom)) - (n.top + n.bottom))
								.constrain(H.minHeight || 0, H.maxHeight
												|| 1000000))
			} else {
				if (this.align == "stretchmax") {
					H.setHeight((x - (n.top + n.bottom)).constrain(H.minHeight
									|| 0, H.maxHeight || 1000000))
				} else {
					if (this.align == "middle") {
						s = b - (q + n.top + n.bottom);
						q = r + n.top + (s / 2);
						if (s > 0) {
							H.setPosition(H.x, q)
						}
		...
		if (D = J.getStyle("overflow") && D != "hidden" && !this.adjustmentPass) {
			var a = this.getLayoutTargetSize();
			if (a.width != v.width || a.height != v.height) {
				this.adjustmentPass = true;
				this.onLayout(e, J)
...
Ext.dd.ScrollManager = function() {
	...
	var g = function(m, l) {
		...
		i.el = m;
		i.dir = l;
		var n = (m.ddScrollConfig && m.ddScrollConfig.frequency)
...
Ext.data.Store = Ext.extend(Ext.util.Observable, {
			...
			constructor : function(a) {
				...
				Ext.apply(this, a);
				this.baseParams = Ext.isObject(this.baseParams)
						? this.baseParams
						: {};
...
Ext.list.ColumnResizer = Ext.extend(Ext.util.Observable, {
	...
	handleHdMove : function(j, g) {
		var b = 5, a = j.getPageX(), i = j.getTarget("em", 3, true);
		if (i) {
			var h = i.getRegion(), d = i.dom.style, c = i.dom.parentNode;
...
Ext.extend(Ext.tree.TreeNode, Ext.data.Node, {
			...
			appendChild : function(c) {
				var b, a;
				if (!c.render && !Ext.isArray(c)) {
					c = this.getLoader().createNode(c)
				} else {
					a = !c.parentNode
				}
				b = Ext.tree.TreeNode.superclass.appendChild.call(this, c);
				if (b) {
					this.afterAdd(b, a)
				}
			...
			insertBefore : function(d, a) {
				var b, c;
				if (!d.render) {
					d = this.getLoader().createNode(d)
				} else {
					c = Ext.isObject(d.parentNode)
				}
				b = Ext.tree.TreeNode.superclass.insertBefore.call(this, d, a);
				if (b && a) {
					this.afterAdd(b, c)
				}
				this.ui.updateExpandIcon();
...
Ext.tree.TreeNodeUI.prototype = {
	...
	renderElements : function(e, k, j, l) {
		this.indentMarkup = e.parentNode
				...
				: "";
		var g = Ext.isBoolean(k.checked), b, c = k.href ? k.href : Ext.isGecko
				? ""
				: "#", d = [
...
Ext.form.ComboBox = Ext.extend(Ext.form.TriggerField, {
	...
	initList : function() {
		if (!this.list) {
			var a = "x-combo-list", d = Ext.getDom(this.getListParent()
					|| Ext.getBody()), b = parseInt(Ext.fly(d)
							.getStyle("z-index"), 10);
			if (this.ownerCt && !b) {
				this.findParentBy(function(e) {
							b = parseInt(e.getPositionEl().getStyle("z-index"),
									10);
							return !!b
						})
			}
			...
			if (this.resizable) {
				...
				this.mon(this.resizer, "resize", function(i, e, g) {
							this.maxHeight = g - this.handleHeight
									- this.list.getFrameWidth("tb")
									- this.assetHeight;
							this.listWidth = e;
							this.innerList.setWidth(e
				...
				if (typeof a == "boolean") {
					...
					if (this.rendered) {
						var b = this.getCheckEl().child(
								"input[name=" + this.el.dom.name + "][value="
										+ a + "]", true);
						if (b) {
...
Ext.grid.GridPanel = Ext.extend(Ext.Panel, {
	...
	applyState : function(a) {
		var l = this.colModel, g = a.columns, k = this.store, m, h, j;
		if (g) {
		...
		if (k) {
			m = a.sort;
			if (m) {
				k[k.remoteSort ? "setDefaultSort" : "sort"](m.field,
						m.direction)
			}
			m = a.group;
			if (k.groupBy) {
	...
	processEvent : function(d, h) {
		this.fireEvent(d, h);
		var g = h.getTarget(), c = this.view, j = c.findHeaderIndex(g);
		if (j !== false) {
			this.fireEvent("header" + d, this, j, h)
		} else {
			var i = c.findRowIndex(g), b, a;
			if (i !== false) {
				this.fireEvent("row" + d, this, i, h);
				b = c.findCellIndex(g);
				a = c.findRowBody(g);
				if (b !== false) {
					this.fireEvent("cell" + d, this, i, b, h)
				}
				if (a) {
					this.fireEvent("rowbody" + d, this, i, h)
				}
			} else {
...
Ext.grid.GridView = Ext.extend(Ext.util.Observable, {
	...
	autoExpand : function(b) {
		...
		if (!this.userResized && i.autoExpandColumn) {
			...
			if (d != j) {
				...
				var c = Math.min(Math.max(((j - d) + e), i.autoExpandMin),
						i.autoExpandMax);
				if (c != e) {
	...
	handleHdMove : function(i, d) {
		...
		if (h && !this.headersDisabled) {
			var b = this.splitHandleWidth || 5, g = this.activeHdRegion, a = i
					.getPageX(), c = h.style, j = "";
			if (this.grid.enableColumnResize !== false) {
				if (a - g.left <= b
						&& this.cm.isResizable(this.activeHdIndex - 1)) {
					j = Ext.isAir ? "move" : Ext.isWebKit
							? "e-resize"
							: "col-resize"
				} else {
...
Ext.grid.RowSelectionModel = Ext.extend(Ext.grid.AbstractSelectionModel, {
	...
	onEditorKey : function(n, l) {
		var d = l.getKey(), h, i = this.grid, o = i.lastEdit, j = i.activeEditor, p, o, a, m;
		var b = l.shiftKey;
		if (d == l.TAB) {
			l.stopEvent();
			j.completeEdit();
			...
			if (d == l.ENTER) {
				if (this.moveEditorOnEnter !== false) {
					if (b) {
						h = i.walkCells(o.row - 1, o.col, -1, this.acceptsNav,
								this)
					} else {
						h = i.walkCells(o.row + 1, o.col, 1, this.acceptsNav,
								this)
...
Ext.grid.Column = Ext.extend(Object, {
			...
			setEditor : function(a) {
				...
				if (a) {
					if (!a.isXType) {
						a = Ext.create(a, "textfield")
					}
					if (!a.startEdit) {
						a = new Ext.grid.GridEditor(a)
					}
					this.editor = a
				}
			},
