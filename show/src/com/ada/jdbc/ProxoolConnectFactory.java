...
public class ProxoolConnectFactory implements ConnectFactory {
	private static Log log = LogFactory.getLog(ProxoolConnectFactory.class);
	...
	Connection createConnect(Properties properties) {
		...
		if (null == properties || properties.size() < 4) {
			...
			try {
				...
				String tempString="proxool.example:";
				tempString+= driver ;
				tempString+= ":" ;
				tempString+=url;
				log.info(tempString);
				connection = DriverManager.getConnection(tempString
						, username, password);
