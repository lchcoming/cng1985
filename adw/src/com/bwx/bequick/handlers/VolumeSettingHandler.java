...
public class VolumeSettingHandler extends SettingHandler implements OnSeekBarChangeListener, OnClickListener, OnCancelListener {
	...
	private static final int[] STRING_IDS = {
		Res.string.txt_volume_ringer,
		Res.string.txt_volume_notification,
		Res.string.txt_volume_media,
		Res.string.txt_volume_alarm,
		Res.string.txt_volume_voice_call,
		Res.string.txt_volume_system
	};
	...
	public void onClick(DialogInterface dialog, int whichButton) {
		...
		if (ringerVolume > 0 && ringerMode == AudioManager.RINGER_MODE_SILENT) {
			Toast.makeText(activity, activity.getString(Res.string.msg_not_silent_warning, ringerVolume), Toast.LENGTH_LONG).show();
		} else if (ringerVolume == 0 && ringerMode == AudioManager.RINGER_MODE_NORMAL) {
			Toast.makeText(activity, Res.string.msg_zero_ringer_warning, Toast.LENGTH_LONG).show();
		} else if (notifVolume == 0 && ringerMode == AudioManager.RINGER_MODE_NORMAL) {
			Toast.makeText(activity, Res.string.msg_zero_notification_warning, Toast.LENGTH_LONG).show();
		}
	...
	private Dialog createDialog() {
		...
		final View dialog = factory.inflate(Res.layout.settings_volume_streams, null);
		final ViewGroup parent = (ViewGroup) dialog.findViewById(Res.id.placeholder);
		...
		for (int i=0; i<length; i++) {
			...
			View control = factory.inflate(Res.layout.row_volume, null);
			
			// init text
			TextView text = (TextView) control.findViewById(Res.id.text);
			text.setText(stringIds[i]);

			// init values
			valueViews[i] = (TextView) control.findViewById(Res.id.value);

			// init seekbars
			SeekBar seekBar = (SeekBar) control.findViewById(Res.id.seekbar);
		...
		AlertDialog d = new AlertDialog.Builder(activity)
			.setIcon(Res.drawable.ic_dialog_menu_generic)
			.setTitle(Res.string.txt_volume)
			.setView(dialog)
			.setPositiveButton(Res.string.btn_set, this)
			.setNegativeButton(Res.string.btn_calcel, this)
			.create();
