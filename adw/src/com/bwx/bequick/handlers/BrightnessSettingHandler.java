...
public class BrightnessSettingHandler extends SettingHandler implements OnClickListener, Runnable {
	...
	public void activate(final MainSettingsActivity activity) {
		...
		setting.descr = auto ? activity.getString(Res.string.txt_autobrightness_enabled) : null;
		...
		switch (buttonIndex) {
			...
			case 1: { // value was updated (from MinMaxSliderRenderer)
				...
				if (mode != MODE_MANUAL) {
					...
					new AlertDialog.Builder(mActivity).setIcon(Res.drawable.ic_dialog_menu_generic).setTitle(
							Res.string.txt_volume).setMessage(Res.string.msg_disable_autobrightness).setPositiveButton(
							Res.string.btn_yes, this).setNegativeButton(Res.string.btn_no, this).create().show();
	...
	public void onSwitched(boolean switched) {
		...
		setting.descr = switched ? activity.getString(Res.string.txt_autobrightness_enabled) : null;
