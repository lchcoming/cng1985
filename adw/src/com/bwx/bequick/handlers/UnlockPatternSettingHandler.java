...
public class UnlockPatternSettingHandler extends SystemPropertySettingHandler implements OnClickListener {
	...
	public void activate(MainSettingsActivity activity) {
		...
		try {
			...
			updateSetting(false, false, Res.string.txt_set_unlock_pattern);
		}
	}
	...
	public void onSwitched(boolean isSwitched) {
		if (isSwitched) {
			...
			new AlertDialog.Builder(mActivity).setIcon(Res.drawable.ic_dialog_menu_generic).setTitle(
					Res.string.txt_unlock_pattern).setMessage(Res.string.msg_which_unlock_pattern).setPositiveButton(
					Res.string.btn_current_unlock_pattern, this).setNegativeButton(Res.string.btn_new_unlock_pattern, this)
					.create().show();
