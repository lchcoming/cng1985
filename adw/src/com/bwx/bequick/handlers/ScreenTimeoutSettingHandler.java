...
public class ScreenTimeoutSettingHandler extends SettingHandler implements OnItemClickListener {
	...
	private void updateUIByValue(int value) {
		...
		if (value <= 0) {
			// never
			descr = mActivity.getString(Res.string.txt_screen_timeout_value_never);
			checked = false;
		} else if (value < 60000) {
			// seconds
			descr = mActivity.getString(Res.string.txt_screen_timeout_value_seconds, value / 1000);
		} else {
			int minutes = value / 60000;
			descr = minutes == 1 
				? mActivity.getString(Res.string.txt_screen_timeout_value_minute, minutes)
				: mActivity.getString(Res.string.txt_screen_timeout_value_minutes, minutes);
		}
