...
public class RingerSettingHandler extends SettingHandler implements OnItemClickListener {
	...
	private Dialog createDialog() {
		...
		dialog.setContentView(Res.layout.simple_list_view);
		...
		value.put("icon", Res.drawable.ic_silent);
		value.put("text", mActivity.getString(Res.string.txt_ringer_silent));
		values.add(value);
		
		value = new HashMap<String, Object>();
		value.put("icon", Res.drawable.ic_vibro);
		value.put("text", mActivity.getString(Res.string.txt_ringer_vibro));
		values.add(value);
		
		value = new HashMap<String, Object>();
		value.put("icon", Res.drawable.ic_sound);
		value.put("text", mActivity.getString(Res.string.txt_ringer_sound));
		values.add(value);
		
		value = new HashMap<String, Object>();
		value.put("icon", Res.drawable.ic_vibro_sound);
		value.put("text", mActivity.getString(Res.string.txt_ringer_vibrosound));
		values.add(value);
		
		SimpleAdapter adapter = new SimpleAdapter(mActivity, values, 
				Res.layout.row_ringer_mode, 
				new String[] {"icon", "text"}, 
				new int[] {Res.id.icon, Res.id.text});
		
		
		ListView view = (ListView) dialog.findViewById(Res.id.list);
	...
	private void updateState() {
		...
		if (ringer == AudioManager.RINGER_MODE_SILENT) {
			descr = Res.string.txt_ringer_silent;
			icon = Res.drawable.ic_silent;
			silent = true;
		} else if (ringer == AudioManager.RINGER_MODE_NORMAL) {
			if (vibro == AudioManager.VIBRATE_SETTING_ON) {
				descr = Res.string.txt_ringer_vibrosound;
				icon = Res.drawable.ic_vibro_sound;
			} else {
				descr = Res.string.txt_ringer_sound;
				icon = Res.drawable.ic_sound;
			}
		} else {
			descr = Res.string.txt_ringer_vibro;
			icon = Res.drawable.ic_vibro;
		}
