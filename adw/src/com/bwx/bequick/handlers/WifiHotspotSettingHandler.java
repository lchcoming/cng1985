...
public class WifiHotspotSettingHandler extends SettingHandler {
	...
	*/
	...
	private void updateState(int wifiApState) {
		...
		int stateTextId = Res.string.txt_status_unknown;
		
		switch (wifiApState) {
			case WifiApManager.WIFI_AP_STATE_DISABLING: {
				enabled = false;
				checked = true;
				stateTextId = Res.string.txt_status_turning_off;
				break;
			}
			case WifiApManager.WIFI_AP_STATE_DISABLED: {
				enabled = true;
				checked = false;
				stateTextId = Res.string.txt_status_turned_off;
				break;
			}
			case WifiApManager.WIFI_AP_STATE_ENABLING: {
				enabled = false;
				checked = false;
				stateTextId = Res.string.txt_status_turning_on;
				break;
			}
			case WifiApManager.WIFI_AP_STATE_ENABLED: {
				enabled = true;
				checked = true;
				stateTextId = Res.string.txt_status_turned_on;
				break;
			}
			case WifiApManager.WIFI_AP_STATE_FAILED: {
				enabled = true;
				checked = false;
				stateTextId = Res.string.txt_net_status_failed;
				break;
			}
		}
