...
public class AirplaneModeSettingHandler extends SettingHandler {
	...
	public void onSwitched(final boolean isSwitched) {
		...
		if (isSwitched) {
			...
			if (noConfirm) {
				...
				builder.setTitle(Res.string.airmode_title)
				.setMessage(Res.string.msg_switch_to_air_mode)
				.setPositiveButton(Res.string.btn_yes, new DialogInterface.OnClickListener() {
					public void onClick(DialogInterface dialog, int which) {
						dialog.dismiss();
						setAirMode(isSwitched);
					}
				}).setNegativeButton(Res.string.btn_no, new DialogInterface.OnClickListener() {
					public void onClick(DialogInterface dialog, int which) {
						dialog.dismiss();
					}
				}).create().show();
	...
	private void updateState() {
		...
		setting.descr = mActivity.getString(isEnabled ? Res.string.txt_status_turned_on : Res.string.txt_status_turned_off);
		setting.updateView();
	}
