...
public class MobileDataSettingHandler2 extends SettingHandler {
	...
	private void switchMobileDataDirectly(boolean enable) {
		...
		if (enable && !mobileDataAllowed) {
			...
			Toast.makeText(mActivity, Res.string.txt_enable_data_access_first, Toast.LENGTH_LONG).show();
		...
		if (enable) {
			if (++mTryCounter > 2) {
				// show 2G hint
				Toast.makeText(mActivity, Res.string.msg_2g_hint, Toast.LENGTH_LONG).show();
				if (mTryCounter > 3) {
					onSelected(0); // open system settings
				}
			} else {
	...
	void updateState(State state) {
		...
		switch (state) {
			case CONNECTED: s.checked = true; s.enabled = true; resId = Res.string.txt_net_status_connected; break;
			case CONNECTING: s.checked = false; s.enabled = false; resId = Res.string.txt_net_status_connecting; break;
			case DISCONNECTED: s.checked = false; s.enabled = true; resId = Res.string.txt_net_status_disconnected; break;
			case DISCONNECTING: s.checked = true; s.enabled = false; resId = Res.string.txt_net_status_disconnecting; break;
			default: s.checked = s.enabled = false; resId = Res.string.txt_status_unknown; break;
		}
