...
public class MobileDataSettingHandler extends SettingHandler {
	...
	public void onSwitched(boolean enabled) {
		...
		if (!enabled && ApnControl.getApnState(mActivity, mPrefs) == STATE_MMS_ONLY) {
			...
			builder.setIcon(Res.drawable.ic_dialog_menu_generic).setTitle(Res.string.txt_apn_control)
				.setMessage(Res.string.msg_mms_only)
				.setPositiveButton(Res.string.btn_disable,
				...
				.setNeutralButton(Res.string.btn_calcel,
					new DialogInterface.OnClickListener() {
						public void onClick(DialogInterface dialog, int which) {
							dialog.dismiss();
						}
	...
	private void setApnState(boolean enabled) {
		...
		if (enabled) {
			Toast.makeText(mActivity, Res.string.msg_enabling_mobile_data, Toast.LENGTH_SHORT).show();
		}
		...
		if (!wasShown) {
			AlertDialog.Builder builder = new AlertDialog.Builder(mActivity);
			builder.setIcon(Res.drawable.ic_dialog_menu_generic).setTitle(Res.string.txt_apn_control).setMessage(
					Res.string.msg_disabled_warning).setNeutralButton(android.R.string.ok,
					new DialogInterface.OnClickListener() {
						public void onClick(DialogInterface dialog, int which) {
	...
	private void updateState(int state) {
		...
		String on = getString(Res.string.txt_on);
		String off = getString(Res.string.txt_off);
		
		switch (state) {
			case STATE_OFF:
				setting.checked = false; 
				setting.enabled = true;
				setting.descr = getString(Res.string.txt_apn_on_off_status, off, disableMMS ? off : on);
				break;
			case STATE_ON: 
			case STATE_MMS_ONLY:
				setting.checked = true;
				setting.enabled = true;
				setting.descr = getString(Res.string.txt_apn_on_off_status, on, on);
				break;
			case STATE_NO_APNS:
			default:
				setting.checked = false; // keep it enabled that users don't worry
				setting.enabled = false;
				setting.descr = getString(Res.string.txt_no_apn);
				break;
		}
