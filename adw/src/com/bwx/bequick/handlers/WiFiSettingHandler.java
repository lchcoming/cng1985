...
public class WiFiSettingHandler extends SettingHandler {
	...
	class WiFiStateReceiver extends BroadcastReceiver {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (WifiManager.WIFI_STATE_CHANGED_ACTION.equals(intent.getAction())) {
				...
				switch (state) {
					case CONNECTING:
						descr = mActivity.getString(Res.string.txt_net_status_connecting);
						break;
					case OBTAINING_IPADDR:
						descr = mActivity.getString(Res.string.txt_net_status_obtainingip);
						break;
					case CONNECTED:
						// String name =
						// intent.getStringExtra(WifiManager.EXTRA_BSSID);
						String name = getWiFiManager().getConnectionInfo().getSSID();
						descr = mActivity.getString(Res.string.txt_net_status_connected_to, name);
						break;
					case FAILED:
						descr = mActivity.getString(Res.string.txt_net_status_failed);
						break;
					case DISCONNECTING:
						descr = mActivity.getString(Res.string.txt_net_status_disconnecting);
						break;
					case DISCONNECTED:
						descr = mActivity.getString(Res.string.txt_net_status_disconnected);
						break;
				}
	...
	private void updateWiFiState(int wifiState) {
		...
		case WifiManager.WIFI_STATE_DISABLING:
			state = Res.string.txt_status_turning_off;
			actionState = STATE_INPROCESS;
			break;

		case WifiManager.WIFI_STATE_DISABLED:
			state = Res.string.txt_status_turned_off;
			actionState = STATE_DONE;
			break;

		case WifiManager.WIFI_STATE_ENABLING:
			state = Res.string.txt_status_turning_on;
			actionState = STATE_INPROCESS;
			break;

		case WifiManager.WIFI_STATE_ENABLED:
			state = Res.string.txt_status_turned_on;
			actionState = STATE_DONE;
			break;

		default:
			state = Res.string.txt_status_unknown;
			actionState = STATE_DONE;
			break;
		}
