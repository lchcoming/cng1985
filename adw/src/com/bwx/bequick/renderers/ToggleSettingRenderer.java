...
public class ToggleSettingRenderer implements SettingRenderer, OnClickListener, OnLongClickListener {
	...
	public View getView(LayoutInflater inflater, Setting setting, View convertView, Context context) {
		...
		mView = convertView = inflater.inflate(Res.layout.row_setting_toggle, null); // inflate view of needed type
		
		// re-initialize views
		mTitle = (TextView) convertView.findViewById(Res.id.title_view);
		mDescr = (TextView) convertView.findViewById(Res.id.descr_view);
		mToggle = (ToggleButton) convertView.findViewById(Res.id.clicable_right);
		mIcon = (ImageView) convertView.findViewById(Res.id.icon_view);
		
		ImageButton button = (ImageButton) convertView.findViewById(Res.id.popup_button);
		button.setVisibility(setting.hasPopup ? View.VISIBLE : View.GONE);
		LinearLayout clicableLeft = (LinearLayout) convertView.findViewById(Res.id.clickable_left);
