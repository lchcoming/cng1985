...
public class SwitchableSliderRenderer implements SettingRenderer, OnClickListener {
	...
	public View getView(LayoutInflater inflater, Setting setting, View convertView, Context context) {
		...
		LinearLayout view = /*mView =*/ (LinearLayout) inflater.inflate(Res.layout.row_switchable_slider_view, null);
		
		ViewSwitcher switcher = mSwitcher = (ViewSwitcher) view.findViewById(Res.id.switcher);
		switcher.setOutAnimation(AnimationUtils.loadAnimation(mContext, Res.anim.slide_out_down));
		switcher.setInAnimation(AnimationUtils.loadAnimation(mContext, Res.anim.slide_in_up));
		switcher.setAnimateFirstView(false);
		
		ToggleButton toggle = mToggle = (ToggleButton) view.findViewById(Res.id.toggle);
