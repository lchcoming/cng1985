...
public class StatusBarIntegrationReceiver extends BroadcastReceiver {
	...
	private synchronized void sendNotification(Context context, int status, int appearance, boolean inverse) {
		...
		if (notification == null) {
			...
			RemoteViews view = notification.contentView = new RemoteViews(context.getPackageName(), Res.layout.status_bar_event);

			// update view color
			view.setImageViewResource(Res.id.image1, inverse ? Res.drawable.ic_logo_white : Res.drawable.ic_logo_black);
			int color = inverse ? Color.WHITE : Color.BLACK;
			view.setTextColor(Res.id.text1, color);
			view.setTextColor(Res.id.text2, color);
		...
		if (status == Constants.STATUS_NO_INTEGRATION) {
			...
			notification.icon = visible 
				? (status == Constants.STATUS_BLACK_ICON ? Res.drawable.ic_logo_black : Res.drawable.ic_logo_white) 
				: sdk9OrLater ? Res.drawable.ic_placeholder : -1;
