...
public class LayoutSettingsAdapter extends BaseAdapter implements OnClickListener {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			convertView = mInflater.inflate(Res.layout.row_layout_placeholder, null);
		}
		...
		if (id == Setting.GROUP_VISIBLE || id == Setting.GROUP_HIDDEN) {
			...
			View view = placeholder.findViewById(Res.id.separator);
			if (view == null) {
				placeholder.removeAllViews();
				view = mInflater.inflate(Res.layout.row_layout_separator, null);
				placeholder.addView(view, FILL_PARRENT);
			}
			
			TextView text = (TextView) view.findViewById(Res.id.title);
			...
			View view = placeholder.findViewById(Res.id.settings);
			if (view == null) {
				placeholder.removeAllViews();
				view = mInflater.inflate(Res.layout.row_layout_setting, null);
				placeholder.addView(view, FILL_PARRENT);
			}
			
			// initialize view
			ImageView image = (ImageView) view.findViewById(Res.id.icon_view);
			image.setImageResource(setting.iconId);
			
			TextView text = (TextView) view.findViewById(Res.id.title_view);
			text.setText(setting.titleId);

			ImageButton button = (ImageButton) view.findViewById(Res.id.wrench);
