...
public class CommonPrefs extends BasePrefs implements OnClickListener, OnPreferenceClickListener {
	...
	public void onSharedPreferenceChanged(SharedPreferences prefs, String key) {
		...
		if (PREF_STATUSBAR_INTEGRATION.equals(key) || PREF_APPEARANCE.equals(key)
			...
			if ("1".equals(value)) { // this is led flashlight
				...
				if (type > -1) {
					// show usage warning
					createDialog(Res.string.txt_flashlight, Res.string.msg_flashlight_led_usage, true).show();
				} else {
					// set property back
					activateScreenFlashlight();
					// show not supported error
					createDialog(Res.string.txt_flashlight, Res.string.msg_flashlight_not_supported, false).show();
				}
	...
	private Dialog createDialog(int titleId, int textId, boolean twoButtons) {
		Builder builder = new AlertDialog.Builder(this).setIcon(Res.drawable.ic_dialog_menu_generic).setTitle(titleId)
				.setMessage(textId);
		if (twoButtons) {
			builder.setPositiveButton(Res.string.btn_accept, this);
			builder.setNegativeButton(Res.string.btn_calcel, this);
		} else {
			builder.setNeutralButton(Res.string.btn_close, this);
		}
	...
	protected Dialog onCreateDialog(int id) {
		...
		if (id == 0) {
			Builder builder = new AlertDialog.Builder(this).setIcon(Res.drawable.ic_dialog_menu_generic).setTitle(
					Res.string.pref_about);
			View view = getLayoutInflater().inflate(Res.layout.about, null);
			TextView info = (TextView) view.findViewById(Res.id.about_info);
			info.setText(Html.fromHtml(getString(Res.string.about_info, getVersionNumber(this))));
			builder.setView(view);

			builder.setNegativeButton(Res.string.btn_close, new OnClickListener() {
				public void onClick(DialogInterface dialog, int which) {
					dialog.dismiss();
				}
			});
