...
function FTB_InsertImage(ftbName) { 
	...
	//alert(ImageGalleryPath);
	//var image=showModalDialog("ftb.imagegallery.aspx",window,'dialogWidth:500px; dialogHeight:400px;help:0;status:0;resizeable:1;');
	var imgArr = showModalDialog("ftb.imagegallery.aspx",window,'dialogWidth:560px; dialogHeight:500px;help:0;status:0;resizeable:1;');
	if (imgArr != null) {
		imagestring = "<IMG SRC='" + imgArr['filename'] + "' HEIGHT=" + imgArr['height'] + ' WIDTH=' + imgArr['width'] + ' BORDER=0>';
		FTB_InsertText(ftbName,imagestring);
	} else {
		//alert(""ÄúÃ»ÓÐÑ¡ÔñÍ¼Æ¬¡£"");
	}
}
...
function FTB_InsertQQEmoticon(ftbName) {
			if (!isIE) 
			{
				alert("Not support!");
			}
...
function FTB_InsertMSNEmoticon(ftbName) {
			if (!isIE) 
			{
				alert("Not support!");
			}
