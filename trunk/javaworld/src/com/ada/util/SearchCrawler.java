	...
	/* disallowListCache缓存robot不允许搜索的URL。 Robot协议在Web站点的根目录下设置一个robots.txt文件, 
	 ...
	 // 搜索下载Web页面的内容，判断在该页面内有没有指定的搜索字符串  
	  ...
	  public static void main(String[] args) {  
	    ...
	    //http://www.sina.com  20 java 
	    SearchCrawler crawler = new SearchCrawler("http://www.sina.com",20,"足球");  
	    Thread  search=new Thread(crawler);  
	    System.out.println("Start searching...");  
	    System.out.println("result:");  
	   // search.start();  
	    Pattern p =Pattern.compile("<a\\s+href\\s*=\\s*\"?(.*?)[\"|>]",Pattern.CASE_INSENSITIVE);  
	    Matcher m = p.matcher("	<a href=\"aadd.html\">aa</a> <a href=\"aadd.html\">aa</a>");
	    boolean b = m.matches();
	    while(m.find()){
	    	System.out.println(m.group(1));  
	    }
	    Pattern pp =Pattern.compile("(a(b){0,1}){1,}",Pattern.CASE_INSENSITIVE);  
	    Matcher mm = pp.matcher("aba abab aabbb bf");
