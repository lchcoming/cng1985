		...
		if (plugin != null) {
			...
			if (!doNotInstallOption.getValue()) {
				...
				if (message != null) {
					...
					final View view = activity.getLayoutInflater().inflate(Res.layout.plugin_dialog, null, false);
					((TextView)view.findViewById(Res.id.plugin_dialog_text)).setText(message);
					final CheckBox checkBox = (CheckBox)view.findViewById(Res.id.plugin_dialog_checkbox);
