...
public class AddCustomCatalogActivity extends Activity {
	...
	public void onCreate(Bundle icicle) {
		...
		setContentView(Res.layout.add_custom_catalog);

		myResource = ZLResource.resource("dialog").getResource("CustomCatalogDialog");

		setTitle(myResource.getResource("title").getValue());

		setTextFromResource(Res.id.add_custom_catalog_title_label, "catalogTitle");
		setTextFromResource(Res.id.add_custom_catalog_url_label, "catalogUrl");
		setTextFromResource(Res.id.add_custom_catalog_summary_label, "catalogSummary");
		setTextFromResource(Res.id.add_custom_catalog_title_example, "catalogTitleExample");
		setTextFromResource(Res.id.add_custom_catalog_url_example, "catalogUrlExample");
		setTextFromResource(Res.id.add_custom_catalog_summary_example, "catalogSummaryExample");

		setupButton(
			Res.id.add_custom_catalog_ok_button, "ok", new View.OnClickListener() {
				public void onClick(View view) {
					final InputMethodManager imm =
						(InputMethodManager)getSystemService(INPUT_METHOD_SERVICE);
					imm.hideSoftInputFromWindow(findViewById(Res.id.add_custom_catalog_url).getWindowToken(), 0);
					imm.hideSoftInputFromWindow(findViewById(Res.id.add_custom_catalog_title).getWindowToken(), 0);
					imm.hideSoftInputFromWindow(findViewById(Res.id.add_custom_catalog_summary).getWindowToken(), 0);
					onOkButton();
				}
			}
		);
		setupButton(
			Res.id.add_custom_catalog_cancel_button, "cancel", new View.OnClickListener() {
				public void onClick(View view) {
					finish();
				}
			}
		...
		if (myLink != null) {
			setTextById(Res.id.add_custom_catalog_url, myLink.getUrl(UrlInfo.Type.Catalog));
			setTextById(Res.id.add_custom_catalog_title, myLink.getTitle());
			setTextById(Res.id.add_custom_catalog_summary, myLink.getSummary());
	...
	private void onOkButton() {
		final String textUrl = getTextById(Res.id.add_custom_catalog_url);
		if (isEmptyString(textUrl)) {
			setErrorByKey("urlIsEmpty");
			return;
		}

		final String title = getTextById(Res.id.add_custom_catalog_title);
		final String summary = getTextById(Res.id.add_custom_catalog_summary);
		Uri uri = null;
		try {
	...
	private void setExtraFieldsVisibility(boolean show) {
		...
		runOnUiThread(new Runnable() {
			public void run() {
				findViewById(Res.id.add_custom_catalog_title_group).setVisibility(visibility);
				findViewById(Res.id.add_custom_catalog_summary_group).setVisibility(visibility);
			}
		});
	}
	...
	private void setErrorText(final String errorText) {
		runOnUiThread(new Runnable() {
			public void run() {
				final TextView errorView = (TextView)findViewById(Res.id.add_custom_catalog_error);
				if (errorText != null) {
	...
	private void loadInfoByUri(Uri uri) {
		...
		setTextById(Res.id.add_custom_catalog_url, textUrl);
