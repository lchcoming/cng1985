...
public class NetworkBookInfoActivity extends Activity implements NetworkView.EventListener {
	...
	protected void onCreate(Bundle icicle) {
		...
		myMainView = getLayoutInflater().inflate(Res.layout.network_book, null, false);
	...
	private final void setupDescription() {
		setTextFromResource(Res.id.network_book_description_title, "description");

		String description = myBook.Summary;
		if (description == null) {
			description = myResource.getResource("noDescription").getValue();
		}
		setTextById(Res.id.network_book_description, description);
	}
	...
	private final void setupExtraLinks() {
		...
		if (extraLinks.isEmpty()) {
			findViewById(Res.id.network_book_extra_links_title).setVisibility(View.GONE);
			findViewById(Res.id.network_book_extra_links).setVisibility(View.GONE);
		} else {
			setTextFromResource(Res.id.network_book_extra_links_title, "extraLinks");
			final LinearLayout extraLinkSection =
				(LinearLayout)findViewById(Res.id.network_book_extra_links);
			...
			for (UrlInfo info : extraLinks) {
				...
				linkView = inflater.inflate(Res.layout.extra_link_item, extraLinkSection, false);
				...
				((TextView)linkView.findViewById(Res.id.extra_link_title)).setText(relatedInfo.Title);
				extraLinkSection.addView(linkView);
			}
			linkView.findViewById(Res.id.extra_link_divider).setVisibility(View.GONE);
		}
	}
	...
	private void setPairLabelTextFromResource(int id, String resourceKey) {
		...
		((TextView)layout.findViewById(Res.id.book_info_key))
			.setText(myResource.getResource(resourceKey).getValue());
	}

	private void setPairValueText(int id, CharSequence text) {
		final LinearLayout layout = (LinearLayout)findViewById(id);
		((TextView)layout.findViewById(Res.id.book_info_value)).setText(text);
	}

	private void setupInfo() {
		setTextFromResource(Res.id.network_book_info_title, "bookInfo");

		setPairLabelTextFromResource(Res.id.network_book_title, "title");
		setPairLabelTextFromResource(Res.id.network_book_authors, "authors");
		setPairLabelTextFromResource(Res.id.network_book_series_title, "series");
		setPairLabelTextFromResource(Res.id.network_book_series_index, "indexInSeries");
		setPairLabelTextFromResource(Res.id.network_book_tags, "tags");
		setPairLabelTextFromResource(Res.id.network_book_catalog, "catalog");

		setPairValueText(Res.id.network_book_title, myBook.Title);

		if (myBook.Authors.size() > 0) {
			findViewById(Res.id.network_book_authors).setVisibility(View.VISIBLE);
			...
			setPairValueText(Res.id.network_book_authors, authorsText);
		} else {
			findViewById(Res.id.network_book_authors).setVisibility(View.GONE);
		}

		if (myBook.SeriesTitle != null) {
			findViewById(Res.id.network_book_series_title).setVisibility(View.VISIBLE);
			setPairValueText(Res.id.network_book_series_title, myBook.SeriesTitle);
			...
			if (indexInSeries > 0) {
				...
				setPairValueText(Res.id.network_book_series_index, seriesIndexString);
				findViewById(Res.id.network_book_series_index).setVisibility(View.VISIBLE);
			} else {
				findViewById(Res.id.network_book_series_index).setVisibility(View.GONE);
			}
		} else {
			findViewById(Res.id.network_book_series_title).setVisibility(View.GONE);
			findViewById(Res.id.network_book_series_index).setVisibility(View.GONE);
		}

		if (myBook.Tags.size() > 0) {
			findViewById(Res.id.network_book_tags).setVisibility(View.VISIBLE);
			...
			setPairValueText(Res.id.network_book_tags, tagsText);
		} else {
			findViewById(Res.id.network_book_tags).setVisibility(View.GONE);
		}

		setPairValueText(Res.id.network_book_catalog, myBook.Link.getTitle());
	}

	private final void setupCover() {
		final View rootView = findViewById(Res.id.network_book_root);
		final ImageView coverView = (ImageView)findViewById(Res.id.network_book_cover);
	...
	private final void setupButtons() {
		...
		final int buttons[] = new int[] {
				Res.id.network_book_button0,
				Res.id.network_book_button1,
				Res.id.network_book_button2,
				Res.id.network_book_button3,
		};
		...
		findViewById(Res.id.network_book_left_spacer).setVisibility(skipSecondButton ? View.VISIBLE : View.GONE);
		findViewById(Res.id.network_book_right_spacer).setVisibility(skipSecondButton ? View.VISIBLE : View.GONE);
		if (skipSecondButton) {
