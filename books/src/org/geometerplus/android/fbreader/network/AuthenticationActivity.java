...
public class AuthenticationActivity extends Activity {
	...
	public void onCreate(Bundle icicle) {
		...
		setContentView(Res.layout.authentication);
		...
		if (area != null && !"".equals(area)) {
			findTextView(Res.id.authentication_subtitle).setText(area);
		} else {
			findTextView(Res.id.authentication_subtitle).setVisibility(View.GONE);
		}
		final TextView warningView = findTextView(Res.id.authentication_unencrypted_warning);
		...
		findTextView(Res.id.authentication_username_label).setText(
			myResource.getResource("login").getValue()
		);
		findTextView(Res.id.authentication_password_label).setText(
		...
		if (showSignupLink) {
			findViewById(Res.id.authentication_signup_box).setVisibility(View.VISIBLE);
			final TextView signupView = (TextView)findViewById(Res.id.authentication_signup);
			...
			findViewById(Res.id.authentication_signup_box).setVisibility(View.GONE);
		}

		final ZLResource buttonResource = ZLResource.resource("dialog").getResource("button");

		final Button okButton = findButton(Res.id.authentication_ok_button);
		...
		okButton.setOnClickListener(new Button.OnClickListener() {
			public void onClick(View v) {
				...
				data.putExtra(
					PASSWORD_KEY,
					findTextView(Res.id.authentication_password).getText().toString()
				);
		...
		final Button cancelButton = findButton(Res.id.authentication_cancel_button);
