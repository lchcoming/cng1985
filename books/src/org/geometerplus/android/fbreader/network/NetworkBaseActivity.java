...
abstract class NetworkBaseActivity extends ListActivity implements NetworkView.EventListener {
	...
	private void setupCover(final ImageView coverView, NetworkTree tree, int width, int height) {
		if (tree instanceof AddCustomCatalogItemTree) {
			coverView.setImageResource(Res.drawable.ic_list_plus);
			return;
		}
		if (tree instanceof SearchItemTree) {
			coverView.setImageResource(Res.drawable.ic_list_searchresult);
			return;
		}
		...
		if (coverBitmap != null) {
			...
			coverView.setImageResource(Res.drawable.ic_list_library_book);
		} else {
			coverView.setImageResource(Res.drawable.ic_list_library_books);
		}
	}
	...
	protected View setupNetworkTreeItemView(View convertView, final ViewGroup parent, NetworkTree tree) {
		final View view = (convertView != null) ? convertView :
			LayoutInflater.from(parent.getContext()).inflate(Res.layout.network_tree_item, parent, false);

		((TextView)view.findViewById(Res.id.network_tree_item_name)).setText(tree.getName());
		((TextView)view.findViewById(Res.id.network_tree_item_childrenlist)).setText(tree.getSecondString());
		...
		final ImageView coverView = (ImageView)view.findViewById(Res.id.network_tree_item_icon);
		...
		final ImageView statusView = (ImageView)view.findViewById(Res.id.network_tree_item_status);
