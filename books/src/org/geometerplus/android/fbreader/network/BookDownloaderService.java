...
public class BookDownloaderService extends Service {
	...
	private Notification createDownloadProgressNotification(String title) {
		final RemoteViews contentView = new RemoteViews(getPackageName(), Res.layout.download_notification);
		contentView.setTextViewText(Res.id.download_notification_title, title);
		contentView.setTextViewText(Res.id.download_notification_progress_text, "");
		contentView.setProgressBar(Res.id.download_notification_progress_bar, 100, 0, true);
		...
		final Handler progressHandler = new Handler() {
			public void handleMessage(Message message) {
				...
				if (progress < 0) {
					...
					contentView.setTextViewText(Res.id.download_notification_progress_text, "" + progress + "%");
					contentView.setProgressBar(Res.id.download_notification_progress_bar, 100, progress, false);
				}
				final NotificationManager notificationManager = (NotificationManager) getSystemService(NOTIFICATION_SERVICE);
				notificationManager.notify(notificationId, progressNotification);
			}
