...
public class BookInfoActivity extends Activity {
	...
	protected void onCreate(Bundle icicle) {
		...
		myHideOpenButton = getIntent().getBooleanExtra(HIDE_OPEN_BUTTON_KEY,
				false);
		...
		setContentView(Res.layout.book_info);
	}
	...
	protected void onStart() {
        ...
        */
		if (myHideOpenButton) {
			findButton(Res.id.book_info_button_open).setVisibility(View.GONE);
		} else {
			setupButton(Res.id.book_info_button_open, "openBook",
		...
		setupButton(Res.id.book_info_button_edit, "editInfo",
				new View.OnClickListener() {
					public void onClick(View view) {
						startActivityForResult(new Intent(
								getApplicationContext(),
								EditBookInfoActivity.class).putExtra(
								CURRENT_BOOK_PATH_KEY, myFile.getPath()), 1);
					}
				});
		setupButton(Res.id.book_info_button_reload, "reloadInfo",
				new View.OnClickListener() {
					public void onClick(View view) {
						if (book != null) {
		...
		final View root = findViewById(Res.id.book_info_root);
		root.invalidate();
		root.requestLayout();
	}
	...
	private void setupButton(int buttonId, String resourceKey,
	...
	private void setupInfoPair(int id, String key, CharSequence value) {
		...
		((TextView) layout.findViewById(Res.id.book_info_key))
				.setText(myResource.getResource(key).getValue());
		((TextView) layout.findViewById(Res.id.book_info_value)).setText(value);
	}

	private void setupCover(Book book) {
		final ImageView coverView = (ImageView) findViewById(Res.id.book_cover);
	...
	private void setupBookInfo(Book book) {
		((TextView) findViewById(Res.id.book_info_title)).setText(myResource
				.getResource("bookInfo").getValue());

		setupInfoPair(Res.id.book_title, "title", book.getTitle());
		...
		setupInfoPair(Res.id.book_authors, "authors", buffer);

		final SeriesInfo series = book.getSeriesInfo();
		setupInfoPair(Res.id.book_series, "series", (series == null) ? null
				: series.Name);
		...
		setupInfoPair(Res.id.book_series_index, "indexInSeries",
				seriesIndexString);
		...
		setupInfoPair(Res.id.book_tags, "tags", buffer);
		String language = book.getLanguage();
		if (!ZLLanguageUtil.languageCodes().contains(language)) {
			language = ZLLanguageUtil.OTHER_LANGUAGE_CODE;
		}
		setupInfoPair(Res.id.book_language, "language",
				ZLLanguageUtil.languageName(language));
	}

	private void setupAnnotation(Book book) {
		final TextView titleView = (TextView) findViewById(Res.id.book_info_annotation_title);
		final TextView bodyView = (TextView) findViewById(Res.id.book_info_annotation_body);
	...
	private void setupFileInfo(Book book) {
		((TextView) findViewById(Res.id.file_info_title)).setText(myResource
				.getResource("fileInfo").getValue());

		setupInfoPair(Res.id.file_name, "name", book.File.getPath());
		if (ENABLE_EXTENDED_FILE_INFO) {
			setupInfoPair(Res.id.file_type, "type", book.File.getExtension());

			final ZLFile physFile = book.File.getPhysicalFile();
			final File file = physFile == null ? null : new File(
					physFile.getPath());
			if (file != null && file.exists() && file.isFile()) {
				setupInfoPair(Res.id.file_size, "size",
						formatSize(file.length()));
				setupInfoPair(Res.id.file_time, "time",
						formatDate(file.lastModified()));
			} else {
				setupInfoPair(Res.id.file_size, "size", null);
				setupInfoPair(Res.id.file_time, "time", null);
			}
		} else {
			setupInfoPair(Res.id.file_type, "type", null);
			setupInfoPair(Res.id.file_size, "size", null);
			setupInfoPair(Res.id.file_time, "time", null);
		}
	}
	...
	private String formatSize(long size) {
		...
		if (size < kilo) { // less than 1 kilobyte
			return myResource.getResource("sizeInBytes").getValue()
					.replaceAll("%s", String.valueOf(size));
		}
		final String value;
		...
		return myResource.getResource("sizeInKiloBytes").getValue()
				.replaceAll("%s", value);
	}
