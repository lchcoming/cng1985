...
abstract class ZLTreeAdapter extends BaseAdapter implements AdapterView.OnItemClickListener, View.OnCreateContextMenuListener {
	...
	protected final void setIcon(ImageView imageView, ZLTree<?> tree) {
		if (tree.hasChildren()) {
			if (isOpen(tree)) {
				imageView.setImageResource(Res.drawable.ic_list_group_open);
			} else {
				imageView.setImageResource(Res.drawable.ic_list_group_closed);
			}
		} else {
			imageView.setImageResource(Res.drawable.ic_list_group_empty);
		}
