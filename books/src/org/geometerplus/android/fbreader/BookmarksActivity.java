...
public class BookmarksActivity extends TabActivity implements MenuItem.OnMenuItemClickListener {
	...
	public void onCreate(Bundle bundle) {
		...
		final TabHost host = getTabHost();
		LayoutInflater.from(this).inflate(Res.layout.bookmarks, host.getTabContentView(), true);
		...
		if (fbreader.Model != null) {
			...
			myThisBookView = createTab("thisBook", Res.id.this_book);
			new BookmarksAdapter(myThisBookView, myThisBookBookmarks, true);
		} else {
			findViewById(Res.id.this_book).setVisibility(View.GONE);
		}

		myAllBooksView = createTab("allBooks", Res.id.all_books);
		new BookmarksAdapter(myAllBooksView, AllBooksBookmarks, false);

		findViewById(Res.id.search_results).setVisibility(View.GONE);
	}
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		item.setIcon(Res.drawable.ic_menu_search);
		return true;
	}
	...
	void showSearchResultsTab(LinkedList<Bookmark> results) {
		if (mySearchResultsView == null) {
			mySearchResultsView = createTab("searchResults", Res.id.search_results);
	...
	private final class BookmarksAdapter extends BaseAdapter implements AdapterView.OnItemClickListener, View.OnCreateContextMenuListener {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			final View view = (convertView != null) ? convertView :
				LayoutInflater.from(parent.getContext()).inflate(Res.layout.bookmark_item, parent, false);
			final ImageView imageView = (ImageView)view.findViewById(Res.id.bookmark_item_icon);
			final TextView textView = (TextView)view.findViewById(Res.id.bookmark_item_text);
			final TextView bookTitleView = (TextView)view.findViewById(Res.id.bookmark_item_booktitle);

			final Bookmark bookmark = getItem(position);
			if (bookmark == null) {
				imageView.setVisibility(View.VISIBLE);
				imageView.setImageResource(Res.drawable.ic_list_plus);
