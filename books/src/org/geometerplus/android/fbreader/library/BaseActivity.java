...
abstract class BaseActivity extends ListActivity {
	...
	protected void createBookContextMenu(ContextMenu menu, Book book) {
		...
		menu.add(0, OPEN_BOOK_ITEM_ID, 0, myResource.getResource("openBook")
				.getValue());
		menu.add(0, SHOW_BOOK_INFO_ITEM_ID, 0,
				myResource.getResource("showBookInfo").getValue());
		if (LibraryInstance.isBookInFavorites(book)) {
			menu.add(0, REMOVE_FROM_FAVORITES_ITEM_ID, 0, myResource
					.getResource("removeFromFavorites").getValue());
		} else {
			menu.add(0, ADD_TO_FAVORITES_ITEM_ID, 0,
					myResource.getResource("addToFavorites").getValue());
		}
		if ((LibraryInstance.getRemoveBookMode(book) & Library.REMOVE_FROM_DISK) != 0) {
			menu.add(0, DELETE_BOOK_ITEM_ID, 0,
					myResource.getResource("deleteBook").getValue());
		}
	}

	protected View createView(View convertView, ViewGroup parent, String name,
			String summary) {
		final View view = (convertView != null) ? convertView : LayoutInflater
				.from(parent.getContext()).inflate(
						Res.layout.library_tree_item, parent, false);

		((TextView) view.findViewById(Res.id.library_tree_item_name))
				.setText(name);
		((TextView) view.findViewById(Res.id.library_tree_item_childrenlist))
				.setText(summary);
		return view;
	}
	...
	protected ImageView getCoverView(View parent) {
		if (myCoverWidth == -1) {
			parent.measure(ViewGroup.LayoutParams.FILL_PARENT,
		...
		final ImageView coverView = (ImageView) parent
				.findViewById(Res.id.library_tree_item_icon);
	...
	protected Bitmap getCoverBitmap(ZLImage cover) {
		...
		return data != null ? data.getBitmap(2 * myCoverWidth,
				2 * myCoverHeight) : null;
	}
	...
	private void tryToDeleteBook(Book book) {
		...
		new AlertDialog.Builder(this)
				...
				.setNegativeButton(buttonResource.getResource("no").getValue(),
						null).create().show();
	}
