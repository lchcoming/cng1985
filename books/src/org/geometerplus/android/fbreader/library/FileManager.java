...
public final class FileManager extends BaseActivity {
	...
	private final class FileItem {
		...
		public int getIcon() {
			if (getBook() != null) {
				return Res.drawable.ic_list_library_book;
			} else if (myFile.isDirectory()) {
				if (myFile.isReadable()) {
					return Res.drawable.ic_list_library_folder;
				} else {
					return Res.drawable.ic_list_library_permission_denied;
				}
			} else if (myFile.isArchive()) {
				return Res.drawable.ic_list_library_zip;
			} else {
				System.err.println(
					"File " + myFile.getPath() +
